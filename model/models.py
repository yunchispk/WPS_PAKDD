...
class LSTM_AD(nn.Module):
    ...
    def forward(self, x):
        hidden = (torch.rand(1, 1, self.n_hidden, dtype=torch.float64, device=device), torch.randn(1, 1, self.n_hidden, dtype=torch.float64, device=device))
        hidden2 = (torch.rand(1, 1, self.n_feats, dtype=torch.float64, device=device), torch.randn(1, 1, self.n_feats, dtype=torch.float64, device=device))
        outputs = []
...
class DAGMM(nn.Module):
    def __init__(self, feats):
        ...
        self.n_hidden = 16
        self.n_latent = 8
        self.n_window = 6 # DAGMM w_size = 5
        self.n = self.n_feats * self.n_window
...
class OmniAnomaly(nn.Module):
    def __init__(self, feats, win_size):
        ...
        self.n_hidden = 32
        self.n_latent = 8
        self.lstm = nn.GRU(win_size * feats, self.n_hidden, 2)
        self.encoder = nn.Sequential(
    ...
    def forward(self, x, hidden = None):
        ...
        out, hidden = self.lstm(x.view(1, 1, -1), hidden) #RuntimeError: input.size(-1) must be equal to input_size. Expected 38, got 228
        ## Encode
        x = self.encoder(out)
...
class MSCRED(nn.Module):
    ...
    def forward(self, g):
        ## Encode
        z = g.view(1, 1, self.n_feats, self.n_window)#shape '[1, 1, 25, 25]' is invalid for input of size 150
...
class MTAD_GAT(nn.Module):
    ...
    def forward(self, data, hidden):
        hidden = torch.rand(1, 1, self.n_hidden, dtype=torch.float64, device=device) if hidden is not None else hidden
...
class MAD_WGANGP(nn.Module):
    def __init__(self, feats):
        super(MAD_WGANGP, self).__init__()
        self.name = 'MAD_WGANGP'
        self.lr = 0.0001
        self.n_feats = feats
        self.n_hidden = 16
        # self.n_hidden = 16
        self.n_window = 5 # MAD_WGANGP w_size = 5
        self.n = self.n_feats * self.n_window
        # self.n = self.n_feats * self.n_window
        latent_dim = 100
    ...
    def forward(self, g=None, real_d=None, mode='out_cgp'):
        # Generate
        print('fg:')
        print(g.shape)
        print('done:')
        ...
        if 'in_cgp' in mode:
            print('yeah')
            real_score = self.discriminator(real_d.view(real_d.shape[0],-1))#.view(1,-1))
            print('real_score:')
            print(real_score.shape)
            print('done:')
            return real_score#.view(-1)
        z = self.generator(g)#.view(1,-1))
        z = z.view(z.shape[0],*(self.n_window, self.n_feats))
        print('fz:')
        print(z.shape)
        print('done:')
        fake_score = self.discriminator(z.view(z.shape[0],-1))#.view(1,-1))
        # z = z.view(z.shape[0])
        print('fz:')
        print(z.shape)
        print('done:')
...
class MAD_GAN(nn.Module):
    def __init__(self, feats):
        ...
        self.n_feats = feats
        self.n_hidden = 16
        self.n_window = 6 # MAD_GAN w_size = 5
        self.n = self.n_feats * self.n_window
        self.generator = nn.Sequential(
            nn.Flatten(),
            nn.Linear(self.n, self.n_hidden), nn.LeakyReLU(True),
